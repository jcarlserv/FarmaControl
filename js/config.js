...
function preencherRodapeVersao(){
  const html = `${SISTEMA.nome} v${SISTEMA.versao} • Desenvolvido por ${SISTEMA.desenvolvidoPor} • <a href="${SISTEMA.instagramUrl}" target="_blank" rel="noopener">${SISTEMA.instagramHandle}</a>`;
  const a = document.getElementById('rodape-versao-login'); if(a) a.innerHTML = html;
  const b = document.getElementById('rodape-versao-app'); if(b) b.innerHTML = html;
}
preencherRodapeVersao();
